...
published: 2016-04-22 16:46:19
---
最近读了一本讲 js 动画的书 [《JavaScript 网页动画设计》](https://book.douban.com/subject/26694195/)，看了开头就知道是本 [velocity.js](http://www.julian.com/research/velocity/) 的大型广告书，但不得不说，这个动画库可以给日常的动画开发带来许多的便利，在书中我也 get 到了一些动画开发的新思路，推荐一下~
  
<!-- more -->
  
## 一、动画代码优化
* 单独把动画逻辑的 js 抽离成独立的文件
...
```
## 二、动画工作流
### 1. 传统方式：按照动画顺序，完成一个元素的动画后，callback 里嵌套另一个元素的动画
```
...
* 因为每一级都有嵌套，因此代码在横向上会迅速变长，这使得在 IDE 中编辑代码越来越 困难。
...
* `$element2` 的 option 中，设置了 sequenceQueue:false，使得 `$element1` 和 `$element2` 会并行触发
### 3. 设计技巧
#### 3.1 定义全局定时乘数
...
## 三、文本动画
字符串切割的 js 插件：[blast](Julian.com/research/blast)  
...
```
## 四、最佳实践
* 减少 dom 元素重复查询，尽量复用变量
* 监听 scroll 或者 resize 时，对事件句柄进行反跳（[debounce](davidwalsh.name/javascript-debounce-function )）
...
* [SVG 口袋指南](https://github.com/jonitrythall/svgpocketguide)
